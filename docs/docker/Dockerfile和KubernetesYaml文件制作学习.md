# Dockerfile与Kubernetes yaml文件制作学习整理
...
### 1: docker commit 
适用于启动的镜像 做好相关修改之后 推迟，然后根据docker ps -a 找到镜像ID
docker commit -m "Added json gem" -a "Docker Newbee"  镜像ID  新镜像名
-m 是修改信息 -a 是制作作者  （可以不要）

### 2: docker build
使用dockerfile 文件制作镜像
docker build -t 镜像名:Tag  .
-t 是将新镜像命名为<镜像名:Tag>  . 指当前文件夹下 名为 Dockerfile的文件
如果名字不是 Dockerfile  则需要改为 -f  并指定文件位置
...
①、Dockerfile 分为四部分：基础镜像信息、维护者信息、镜像操作指令、容器启动执行指令。
   1)、一开始必须要指明所基于的镜像名称；
       ...
       通过 FROM 指定的镜像，可以是任何有效的基础镜像。FROM 有以下限制：
         ...
         2、在一个 Dockerfile 文件中创建多个镜像时，FROM 可以多次出现。只需在每个新命令 FROM 之前，
             记录提交上次的镜像 ID。
...
## Dockerfile文件格式：
 ...
 /etc/apt/sources.list RUN apt-get update && apt-get install -y nginx RUN echo "\ndaemon off;" >>
...
Dockerfile实践
```
et-dispatcher 镜像制作(jdk1.8)
docker bulid -t et-dispatcher:20191118 -f dispatcherDockerfile
...
CMD /app/bin/start.sh $TYPE $CENTER_CODE


et-web镜像制作(tomcat)
docker bulid -t et-web:20191118 -f etWebDockerfile

FROM davidcaste/alpine-tomcat:jdk8tomcat7
ADD et.war /opt/tomcat/webapps/
ENTRYPOINT ["/bin/sh"]
CMD ["-c","/opt/tomcat/bin/catalina.sh run 2>&1"]

制作redis cluster集群的镜像


制作mysql的镜像 并执行相关sql 并持久化本地数据


制作oracle的镜像 并执行相关sql 并持久化本地数据
